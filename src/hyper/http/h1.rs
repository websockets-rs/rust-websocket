...
use ::hyper::httparse;
use ::hyper::url::Position as UrlPosition;

use ::hyper::buffer::BufReader;
use ::hyper::Error;
use ::hyper::header::{Headers, ContentLength, TransferEncoding};
use ::hyper::header::Encoding::Chunked;
...
impl HttpMessage for Http11Message {
    fn set_outgoing(&mut self, mut head: RequestHead) -> ::hyper::Result<RequestHead> {
    ...
    fn get_incoming(&mut self) -> ::hyper::Result<ResponseHead> {
    ...
    #[inline]
    fn close_connection(&mut self) -> ::hyper::Result<()> {
...
impl<C: NetworkConnector<Stream=S> + Send + Sync, S: NetworkStream + Send>
    ...
    fn connect(&self, host: &str, port: u16, scheme: &str)
        -> ::hyper::Result<Box<NetworkStream + Send>> {
...
fn parse<R: Read, T: TryParse<Subject=I>, I>(rdr: &mut BufReader<R>) -> ::hyper::Result<Incoming<I>> {
    loop {
        ...
        if n == 0 {
            ...
            if buffered == ::hyper::buffer::MAX_BUFFER_SIZE {
