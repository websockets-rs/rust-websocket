...
impl FromStr for StrictTransportSecurity {
    ...
    fn from_str(s: &str) -> ::hyper::Result<StrictTransportSecurity> {
        s.split(';')
            .map(str::trim)
            ...
            .fold(Ok((None, None)), |res, dir| match (res, dir) {
                ...
                (Ok((Some(_), _)), Ok(Directive::MaxAge(_))) => Err(::hyper::Error::Header),
                (Ok((_, Some(_))), Ok(Directive::IncludeSubdomains)) => Err(::hyper::Error::Header),
                (_, Err(_)) => Err(::hyper::Error::Header),
                (res, _) => res
            })
...
impl Header for StrictTransportSecurity {
    ...
    fn parse_header(raw: &[Vec<u8>]) -> ::hyper::Result<StrictTransportSecurity> {
...
mod tests {
    ...
    fn test_parse_max_age_no_value() {
        let h: ::hyper::Result<StrictTransportSecurity> = Header::parse_header(&[b"max-age".to_vec()][..]);
        assert!(h.is_err());
    }
    ...
    fn test_parse_no_max_age() {
        let h: ::hyper::Result<StrictTransportSecurity> = Header::parse_header(&[b"includeSubDomains".to_vec()][..]);
        assert!(h.is_err());
    }

    #[test]
    fn test_parse_max_age_nan() {
        let h: ::hyper::Result<StrictTransportSecurity> = Header::parse_header(&[b"max-age = derp".to_vec()][..]);
        assert!(h.is_err());
    }
