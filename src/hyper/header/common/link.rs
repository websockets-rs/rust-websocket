    ...
    fn parse_header(raw: &[Vec<u8>]) -> ::hyper::Result<Link> {
        ...
        raw.iter()
            ...
            .fold(None, |p, c| match (p, c) {
                ...
                _ => Some(Err(::hyper::Error::Header)),
            })
            .unwrap_or(Err(::hyper::Error::Header))
    }
}
...
impl FromStr for Link {
    ...
    fn from_str(s: &str) -> ::hyper::Result<Link> {
                ...
                if "rel".eq_ignore_ascii_case(link_param_name) {
                    ...
                    if link_header.rel.is_none() {
                        link_header.rel = match link_param_split.next() {
                            None => return Err(::hyper::Error::Header),
                            Some("") =>  return Err(::hyper::Error::Header),
                            Some(s) => {
                                s.trim_matches(|c: char| c == '"' || c.is_whitespace())
                                    .split(' ')
                                    .map(|t| t.trim().parse())
                                    .collect::<Result<Vec<RelationType>, _>>()
                                    .or_else(|_| return Err(::hyper::Error::Header))
                                    .ok()
                            },
                        };
                    }
...
impl FromStr for MediaDesc {
    ...
    fn from_str(s: &str) -> ::hyper::Result<MediaDesc> {
        match s {
...
impl FromStr for RelationType {
    ...
    fn from_str(s: &str) -> ::hyper::Result<RelationType> {
...
fn verify_and_trim(s: &str, b: (u8, u8)) -> ::hyper::Result<&str> {
    let length = s.len();
