...
impl FromStr for ContentRangeSpec {
    ...
    fn from_str(s: &str) -> ::hyper::Result<Self> {
        let res = match split_in_two(s, ' ') {
            Some(("bytes", resp)) => {
                let (range, instance_length) = try!(split_in_two(resp, '/').ok_or(::hyper::Error::Header));

                let instance_length = if instance_length == "*" {
                    None
                } else {
                    Some(try!(instance_length.parse().map_err(|_| ::hyper::Error::Header)))
                };

                let range = if range == "*" {
                    None
                } else {
                    let (first_byte, last_byte) = try!(split_in_two(range, '-').ok_or(::hyper::Error::Header));
                    let first_byte = try!(first_byte.parse().map_err(|_| ::hyper::Error::Header));
                    let last_byte = try!(last_byte.parse().map_err(|_| ::hyper::Error::Header));
