...
impl FromStr for Range {
    ...
    fn from_str(s: &str) -> ::hyper::Result<Range> {
...
impl FromStr for ByteRangeSpec {
    ...
    fn from_str(s: &str) -> ::hyper::Result<ByteRangeSpec> {
        let mut parts = s.splitn(2, "-");

        match (parts.next(), parts.next()) {
            (Some(""), Some(end)) => {
                end.parse().or(Err(::hyper::Error::Header)).map(ByteRangeSpec::Last)
            },
            (Some(start), Some("")) => {
                start.parse().or(Err(::hyper::Error::Header)).map(ByteRangeSpec::AllFrom)
            },
            (Some(start), Some(end)) => {
...
impl Header for Range {
    ...
    fn parse_header(raw: &[Vec<u8>]) -> ::hyper::Result<Range> {
...
fn test_parse_invalid() {
    let r: ::hyper::Result<Range> = Header::parse_header(&[b"bytes=1-a,-".to_vec()]);
    assert_eq!(r.ok(), None);

    let r: ::hyper::Result<Range> = Header::parse_header(&[b"bytes=1-2-3".to_vec()]);
    assert_eq!(r.ok(), None);

    let r: ::hyper::Result<Range> = Header::parse_header(&[b"abc".to_vec()]);
    assert_eq!(r.ok(), None);

    let r: ::hyper::Result<Range> = Header::parse_header(&[b"bytes=1-100=".to_vec()]);
    assert_eq!(r.ok(), None);

    let r: ::hyper::Result<Range> = Header::parse_header(&[b"bytes=".to_vec()]);
    assert_eq!(r.ok(), None);

    let r: ::hyper::Result<Range> = Header::parse_header(&[b"custom=".to_vec()]);
    assert_eq!(r.ok(), None);

    let r: ::hyper::Result<Range> = Header::parse_header(&[b"=1-100".to_vec()]);
    assert_eq!(r.ok(), None);
}
