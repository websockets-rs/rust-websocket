...
use ::hyper::header::{Header, HeaderFormat, MultilineFormatter};
...
impl Item {
    #[inline]
    pub(crate) fn new_raw(data: Vec<Vec<u8>>) -> Item {
        Item {
    ...
    #[inline]
    pub(crate) fn new_typed(ty: Box<HeaderFormat + Send + Sync>) -> Item {
    ...
    #[inline]
    pub(crate) fn raw_mut(&mut self) -> &mut Vec<Vec<u8>> {
        self.raw();
    ...
    pub(crate) fn typed<H: Header + HeaderFormat + Any>(&self) -> Option<&H> {
        let tid = TypeId::of::<H>();
    ...
    pub(crate) fn typed_mut<H: Header + HeaderFormat>(&mut self) -> Option<&mut H> {
        let tid = TypeId::of::<H>();
    ...
    pub(crate) fn write_h1(&self, f: &mut MultilineFormatter) -> fmt::Result {
        match *self.raw {
            Some(ref raw) => {
...
fn parse<H: Header + HeaderFormat>(raw: &Vec<Vec<u8>>) ->
        ::hyper::Result<Box<HeaderFormat + Send + Sync>> {
